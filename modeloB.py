...
def df_to_X_y(df, window_size=15): #window_size referente a quantos dados vamos utilizar
...
plt.title('Gráfico de Previsão de Treinamento - Modelo B')
...
plt.title('Gráfico de Previsão de Validação - Modelo B')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
...
train_results.to_csv('modeloB_train_results.csv', index=False, sep=";", float_format='%.2f')
val_results.to_csv('modeloB_val_results.csv', index=False, sep=";", float_format='%.2f')