...
model1.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=50, callbacks=[cp]) #epoch quantas vezes vai rodar
...
plt.title('Gráfico de Previsão de Treinamento - Modelo D ')
...
plt.title('Gráfico de Previsão de Validação - Modelo D')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
...
train_results.to_csv('modeloD_train_results.csv', index=False, sep=";", float_format='%.2f')
val_results.to_csv('modeloD_val_results.csv', index=False, sep=";", float_format='%.2f')