...
df['Natural'] = df['Natural'].str.replace(',','.').astype(float)
print(df.info())
print(df.head())
...
def df_to_X_y(df, window_size=5, input_cols=['Afluente', 'Defluente', 'Natural']): #window_size referente a quantos dados vamos utilizar
    ...
    for i in range(len(df_as_np)-window_size):
        ...
        X.append(row)
        label = df_as_np[i+window_size][1] #var alvo é a segunda coluna, posição 1
        y.append(label)
    return np.array(X), np.array(y)

window_size = 5
input_cols = ['Afluente', 'Defluente', 'Natural']
...
plt.title('Gráfico de Previsão de Treinamento - Modelo G')
...
plt.title('Gráfico de Previsão de Validação - Modelo G')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
...
plt.plot(test_results['Test Predictions'][:100], label='Teste ')
plt.plot(test_results['Actuals'][:100], label='Real')
plt.title('Gráfico de Previsão de Teste - Modelo G')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
plt.legend(loc='upper left', facecolor='none')
plt.grid()
...
train_results.to_csv('modeloG_train_results.csv', index=False, sep=";", float_format='%.2f')
val_results.to_csv('modeloG_val_results.csv', index=False, sep=";", float_format='%.2f')