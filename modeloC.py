# -*- coding: utf-8 -*-
"""
Created on Tue Feb 28 18:22:32 2023
...
def df_to_X_y(df, window_size=30): #window_size referente a quantos dados vamos utilizar
    df_as_np = df.to_numpy()
    X = []
    y = []
    for i in range(len(df_as_np)-window_size):
        row = [[a] for a in df_as_np[i:i+30]]
        X.append(row)
        label = df_as_np[i+30]
        y.append(label)
...
model1.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, callbacks=[cp]) #epoch quantas vezes vai rodar
...
plt.title('Gráfico de Previsão de Treinamento - Modelo C')
...
plt.title('Gráfico de Previsão de Validação - Modelo C')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
...
plt.plot(test_results['Test Predictions'][:100], label='Teste Previsão')
plt.plot(test_results['Actuals'][:100], label='Teste Real')

plt.legend(loc=0)

train_results.to_csv('modeloC_train_results.csv', index=False, sep=";", float_format='%.2f')
val_results.to_csv('modeloC_val_results.csv', index=False, sep=";", float_format='%.2f')
