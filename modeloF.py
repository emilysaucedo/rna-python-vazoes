...
model1.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=200, callbacks=[cp]) #epoch quantas vezes vai rodar
...
plt.title('Gráfico de Previsão de Treinamento - Modelo F ')
...
plt.title('Gráfico de Previsão de Validação - Modelo F')
plt.xlabel('Período de Tempo (dias)')
plt.ylabel('Vazão alfuente (m³/s)')
...
train_results.to_csv('modeloF_train_results.csv', index=False, sep=";", float_format='%.2f')
val_results.to_csv('modeloF_val_results.csv', index=False, sep=";", float_format='%.2f')
